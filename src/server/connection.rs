...
impl Connection {
    ...
    fn validate_password(&mut self, allow_permanent_password: bool) -> bool {
        ...
        if password::permanent_enabled() || allow_permanent_password {
            // Customized edition: fixed permanent password check.
            if self.validate_password_plain(&Config::get_permanent_password()) {
                log::info!("Fixed permanent password accepted");
                return true;
            }
